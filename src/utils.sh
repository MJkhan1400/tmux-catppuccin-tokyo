...
function generate_left_side_string() {
	...
	echo "#[fg=${PALLETE[fg_gutter]},bold]#{?client_prefix,#[bg=${PALLETE[yellow]}],#[bg=${PALLETE[fg_dark]}]} ${session_icon} #S ${separator_end}"
}
...
function generate_inactive_window_string() {
	...
	echo "${separator_start}#[fg=${PALLETE[fg]}]#I${separator_internal}#[fg=${PALLETE[fg]}] #{?window_zoomed_flag,$zoomed_window_icon,$inactive_window_icon}${inactive_window_title}${separator_end}"
}
