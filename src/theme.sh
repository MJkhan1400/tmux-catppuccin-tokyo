...
theme_variation=$(get_tmux_option "@theme_variation" "mocha")
...
tmux set-option -g status-style "bg=${status_bar_bg},fg=${PALLETE[fg_dark]}"
	...
	for plugin in "${plugins[@]}"; do
		...
		if [ ! -f "${CURRENT_DIR}/plugin/${plugin}.sh" ]; then
			...
			if [ "$plugin" == "datetime" ] || [ "$plugin" == "battery" ]; then
				plugin_output="#[fg=${PALLETE[bg]},bg=${accent_color}]${plugin_execution_string}#[none]"
			else
				plugin_output="#[fg=${PALLETE[bg]},bg=${accent_color}]#($plugin_script_path)#[none]"
			fi
